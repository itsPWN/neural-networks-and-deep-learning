...
def main():
    ...
    f.close()
    book_js = str(Path(__file__).resolve().parent.parent / "book" / "js")
    shutil.copy("initial_gradient.json", book_js + "/initial_gradient.json")
...
def plot_training(epochs, filename, num_layers):
    ...
    book_images = str(Path(__file__).resolve().parent.parent / "book" / "images")
    shutil.copy(fig_filename, book_images + "/" + fig_filename)
    plt.show()
